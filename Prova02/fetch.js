document.addEventListener("DOMContentLoaded", () => {
    const apiUrl = "https://bu.furb.br/mcardoso/progWeb/geraJSON.php";
...
function criarTabela(dados) {
    ...
    const cabecalho = ["ID", "Nome", "Departamento", "Endereço", "E-mail", "Ações"];
    ...
    dados.forEach(item => {
        ...
        img.addEventListener("click", () => {
            const confirmacao = confirm(`Tem certeza de que deseja excluir o item com ID ${item.id}?`);
            if (confirmacao) {
                alert(`O item com ID ${item.id} foi excluído com sucesso!`);
                linha.remove();
            }
        });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    addForm.addEventListener("submit", (event) => {
        ...
        if (confirmacao) {
            ...
            const celulas = [tbody.rows.length + 1, nome, departamento, endereco, email];
            celulas.forEach(celulaTexto => {
                const td = document.createElement("td");
                td.textContent = celulaTexto;
                td.style.padding = "8px";
